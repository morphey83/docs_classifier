...
| **7j document page** ✅ | breadcrumb dropped (it echoed the top nav) — just a domain link above the title. New **"Наборы"** card beside Теги / Обработка / …: the sets the doc is in (each removable) + a picker to add it elsewhere, HTMX in place, shown for every document (previously hidden whenever the domain had no sets). `POST /documents/{id}/remove-from-set`; `add-to-set` returns the doc-body fragment. Теги card = input + the domain's frequent-tag chips underneath with the inbox modal's "chip inactive once present" behaviour. |
