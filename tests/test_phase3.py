...
async def domain(alice):
    return (await alice.post("/api/domains", json={"name": "P3"})).json()
...
async def _upload_png(client, domain_id, name="scan.png"):
    ...
    r = await client.post(
        f"/api/domains/{domain_id}/uploads", files={"file": (name, data, "image/png")}
    )
...
async def test_ocr_extracts_text_and_makes_searchable(alice, domain, fake_ocr):
    ...
    r = await alice.post(f"/api/documents/{doc['id']}/ocr")
    assert r.status_code == 200, r.text
    assert r.json()["ocr_status"] == "pending"  # job is queued

    body = (await alice.get(f"/api/documents/{doc['id']}")).json()  # job has run
    ...
    hit = await alice.get(f"/api/domains/{domain['id']}/documents?q=оплату")
    assert [x["id"] for x in hit.json()["items"]] == [doc["id"]]


async def test_ocr_lang_override(alice, domain, fake_ocr):
    doc = await _upload_png(alice, domain["id"])
    await alice.post(f"/api/documents/{doc['id']}/ocr?lang=deu")
...
async def test_ocr_unsupported_type_422(alice, domain):
    ...
    doc2 = await alice.get(f"/api/documents/{doc['id']}")
...
async def test_ocr_failure_marks_failed(alice, domain, monkeypatch):
    ...
    assert (await alice.get(f"/api/documents/{doc['id']}")).json()["ocr_status"] == "failed"


async def test_auto_ocr_on_upload(alice, domain, fake_ocr):
    d = domain["id"]
    await alice.patch(f"/api/domains/{d}", json={"settings": {"auto_ocr": True}})
    doc = await _upload_png(alice, d)
    got = await alice.get(f"/api/documents/{doc['id']}")
...
async def test_has_ocr_filter(alice, domain, fake_ocr):
    ...
    await alice.post(f"/api/documents/{a['id']}/ocr")

    assert (await alice.get(f"/api/domains/{d}/documents?has_ocr=true")).json()["total"] == 1
    assert (await alice.get(f"/api/domains/{d}/documents?has_ocr=false")).json()["total"] == 1
