...
async def test_set_crud_and_items(alice, domain):
    ...
    s = (await alice.post(f"/api/domains/{d}/sets", json={"name": "Q3 docs"})).json()
    assert s["item_count"] == 0 and s["visibility"] == "private"

    detail = (
        await alice.post(
            f"/api/domains/{d}/sets/{s['id']}/items",
    ...
    detail = (
        await alice.request(
            "DELETE", f"/api/domains/{d}/sets/{s['id']}/items/{docs[0]['id']}"
        )
    ).json()
    assert detail["item_count"] == 1

    listed = (await alice.get(f"/api/domains/{d}/sets")).json()
...
async def test_archive_builds_lazily_and_rebuilds_on_change(alice, domain):
    ...
    s = (
        await alice.post(
            f"/api/domains/{d}/sets",
            json={"name": "pack", "document_ids": [docs[0]["id"], docs[1]["id"]]},
        )
    ).json()

    first = await alice.get(f"/api/domains/{d}/sets/{s['id']}/archive/download")
    assert first.status_code == 202
    assert first.json()["status"] == "building"

    ready = await _download_when_ready(alice, f"/api/domains/{d}/sets/{s['id']}/archive/download")
    assert ready.status_code == 200
    with zipfile.ZipFile(io.BytesIO(ready.content)) as zf:
        assert sum(n.startswith("files/") for n in zf.namelist()) == 2

    status = (await alice.get(f"/api/domains/{d}/sets/{s['id']}/archive")).json()
    assert status["ready"] and status["item_count"] == 2

    # change the set -> next request transparently rebuilds
    await alice.request("DELETE", f"/api/domains/{d}/sets/{s['id']}/items/{docs[1]['id']}")
    after = await alice.get(f"/api/domains/{d}/sets/{s['id']}/archive/download")
    assert after.status_code == 202  # stale, rebuild queued

    ready2 = await _download_when_ready(alice, f"/api/domains/{d}/sets/{s['id']}/archive/download")
...
async def test_set_visibility_between_members(alice, bob, domain):
    ...
    priv = (await alice.post(f"/api/domains/{d}/sets", json={"name": "mine"})).json()
    shared = (
        await alice.post(
            f"/api/domains/{d}/sets", json={"name": "ours", "visibility": "domain"}
        )
    ).json()

    assert (await bob.get(f"/api/domains/{d}/sets/{priv['id']}")).status_code == 404
    assert (await bob.get(f"/api/domains/{d}/sets/{shared['id']}")).status_code == 200
    assert {x["id"] for x in (await bob.get(f"/api/domains/{d}/sets")).json()} == {shared["id"]}

    # editor is not the creator and lacks 'manage' -> cannot edit the shared set
    assert (
        await bob.patch(f"/api/domains/{d}/sets/{shared['id']}", json={"name": "hijack"})
    ).status_code == 403
...
async def test_one_time_link_public_download(alice, domain):
    ...
    s = (
        await alice.post(
            f"/api/domains/{d}/sets", json={"name": "rep", "document_ids": [doc["id"]]}
        )
    ).json()

    link = (
        await alice.post(f"/api/domains/{d}/sets/{s['id']}/links", json={"kind": "one_time"})
    ).json()
...
async def test_permanent_link_needs_write_and_stays_live(alice, bob, domain):
    ...
    bob_me = (await bob.get("/api/auth/me")).json()
    await alice.post(
        f"/api/domains/{d}/members", json={"username": bob_me["username"], "role": "viewer"}
    )
    ...
    assert (
        await bob.post(f"/api/domains/{d}/sets/{s['id']}/links", json={"kind": "permanent"})
    ).status_code == 403
    assert (
        await bob.post(f"/api/domains/{d}/sets/{s['id']}/links", json={"kind": "one_time"})
    ).status_code == 201

    link = (
        await alice.post(
            f"/api/domains/{d}/sets/{s['id']}/links", json={"kind": "permanent"}
        )
    ).json()
    ...
    # revoke -> dead
    assert (await alice.request("DELETE", f"/api/links/{link['id']}")).status_code == 204
...
async def test_public_link_dies_when_owner_loses_download(alice, bob, domain):
    ...
    await alice.patch(
        f"/api/domains/{d}/members/{bob_me['id']}", json={"role": "scanner"}
    )
...
async def test_allow_public_links_toggle(alice, domain):
    ...
    s = (
        await alice.post(
            f"/api/domains/{d}/sets", json={"name": "n", "document_ids": [doc["id"]]}
        )
    ).json()
    await alice.patch(f"/api/domains/{d}", json={"settings": {"allow_public_links": False}})
...
async def test_set_and_archive_hidden_from_non_member(alice, bob, domain):
    ...
    s = (
        await alice.post(
            f"/api/domains/{d}/sets", json={"name": "z", "document_ids": [doc["id"]]}
        )
    ).json()
    assert (await bob.get(f"/api/domains/{d}/sets/{s['id']}")).status_code == 404
    assert (
        await bob.get(f"/api/domains/{d}/sets/{s['id']}/archive/download")
    ).status_code == 404
