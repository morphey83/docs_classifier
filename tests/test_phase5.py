...
async def domain(alice):
    return (await alice.post("/api/domains", json={"name": "P5"})).json()
...
async def test_soft_delete_hide_then_restore(alice, domain):
    ...
    assert (await alice.request("DELETE", f"/api/documents/{doc['id']}")).status_code == 204

    assert (await alice.get(f"/api/domains/{d}/documents")).json()["total"] == 0
    assert (await alice.get(f"/api/domains/{d}/documents?include_trash=true")).json()["total"] == 1
    trash = (await alice.get(f"/api/domains/{d}/trash")).json()
    assert [x["id"] for x in trash["items"]] == [doc["id"]]
    assert trash["items"][0]["deleted_at"] is not None

    restored = await alice.post(f"/api/documents/{doc['id']}/restore")
    assert restored.status_code == 200
    assert restored.json()["deleted_at"] is None
    assert (await alice.get(f"/api/domains/{d}/documents")).json()["total"] == 1
    assert (await alice.get(f"/api/domains/{d}/trash")).json()["total"] == 0
...
async def test_delete_needs_delete_capability(alice, bob, domain):
    ...
    assert (await bob.request("DELETE", f"/api/documents/{doc['id']}")).status_code == 403

    await alice.patch(f"/api/domains/{d}/members/{bob_me['id']}", json={"role": "admin"})
    assert (await bob.request("DELETE", f"/api/documents/{doc['id']}")).status_code == 204
...
async def test_force_purge_is_owner_only(alice, bob, domain):
    ...
    bob_me = (await bob.get("/api/auth/me")).json()
    await alice.post(
        f"/api/domains/{d}/members", json={"username": bob_me["username"], "role": "admin"}
    )
    doc = await _upload(alice, d, "c.txt")
    await alice.request("DELETE", f"/api/documents/{doc['id']}")

    assert (await bob.post(f"/api/domains/{d}/trash/purge")).status_code == 403

    r = await alice.post(f"/api/domains/{d}/trash/purge")
    assert r.status_code == 200 and r.json()["purged"] == 1
    assert (await alice.get(f"/api/documents/{doc['id']}")).status_code == 404
...
async def test_restore_conflict_when_content_now_active(alice, domain):
    ...
    r = await alice.post(f"/api/documents/{doc['id']}/restore")
...
async def test_cleanup_purges_expired_trash_and_gcs_blob(alice, domain, monkeypatch):
    ...
    await alice.request("DELETE", f"/api/documents/{gone['id']}")

    monkeypatch.setattr(settings, "default_trash_retention_days", -1)
    stats = await run_cleanup()

    assert stats["purged_documents"] == 1
    assert (await alice.get(f"/api/documents/{gone['id']}")).status_code == 404
    assert not storage.blob_exists(gone["sha256"])
    assert (await alice.get(f"/api/documents/{kept['id']}/content")).status_code == 200
...
async def test_cleanup_sweeps_orphan_blobs(alice, domain):
    ...
    assert (await alice.get(f"/api/documents/{doc['id']}/content")).status_code == 200
