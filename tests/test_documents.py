...
async def domain(alice):
    return (await alice.post("/api/domains", json={"name": "Docs"})).json()
...
async def test_upload_lands_in_inbox(alice, domain):
    r = await alice.post(f"/api/domains/{domain['id']}/uploads", files=_file())
    ...
    inbox = await alice.get(f"/api/domains/{domain['id']}/inbox")
    assert inbox.json()["count"] == 1


async def test_identical_content_is_deduplicated(alice, domain):
    a = await alice.post(f"/api/domains/{domain['id']}/uploads", files=_file())
    b = await alice.post(f"/api/domains/{domain['id']}/uploads", files=_file())
...
async def test_name_conflict_then_replace_and_new(alice, domain):
    d = domain["id"]
    await alice.post(f"/api/domains/{d}/uploads", files=_file(content=b"v1"))
    conflict = await alice.post(f"/api/domains/{d}/uploads", files=_file(content=b"v2"))
    assert conflict.status_code == 409
    assert conflict.json()["detail"]["error"] == "name_conflict"

    rep = await alice.post(
        f"/api/domains/{d}/uploads?on_conflict=replace", files=_file(content=b"v2")
    )
    assert rep.json()["outcome"] == "replaced"
    assert rep.json()["document"]["version"] == 2

    new = await alice.post(
        f"/api/domains/{d}/uploads?on_conflict=new", files=_file(content=b"v3")
    )
...
async def test_tagging_and_complete_flow(alice, domain):
    d = domain["id"]
    doc = (await alice.post(f"/api/domains/{d}/uploads", files=_file())).json()["document"]

    tagged = await alice.patch(
        f"/api/documents/{doc['id']}/tags", json={"tag_names": ["Контракт", "2024"]}
    )
    ...
    done = await alice.post(f"/api/documents/{doc['id']}/complete")
    assert done.json()["status"] == "tagged"
    assert (await alice.get(f"/api/domains/{d}/inbox")).json()["count"] == 0

    # filter by tag
    found = await alice.get(f"/api/domains/{d}/documents?tags=2024")
...
async def test_download_returns_bytes(alice, domain):
    doc = (
        await alice.post(f"/api/domains/{domain['id']}/uploads", files=_file(content=b"PAYLOAD"))
    ).json()["document"]
    r = await alice.get(f"/api/documents/{doc['id']}/content")
...
async def test_quota_enforced(alice, domain, monkeypatch):
    ...
    r = await alice.post(f"/api/domains/{domain['id']}/uploads", files=_file(content=b"x" * 2048))
