...
async def domain(alice):
    return (await alice.post("/domains", json={"name": "P2"})).json()
...
async def test_archive_upload_extracts_into_inbox(alice, domain):
    ...
    r = await alice.post(
        f"/domains/{d}/uploads", files={"file": ("pack.zip", archive, "application/zip")}
    )
    assert r.status_code == 202, r.text
    batch_id = r.json()["id"]

    # background task ran before the response returned
    detail = await alice.get(f"/domains/{d}/uploads/{batch_id}")
    b = detail.json()
    assert b["status"] == "done"
    assert b["item_count"] == 2
    assert {i["entry_name"] for i in b["items"]} == {"a.txt", "sub/b.txt"}

    docs = await alice.get(f"/domains/{d}/documents")
...
async def test_archive_name_conflict_is_skipped(alice, domain):
    d = domain["id"]
    await alice.post(f"/domains/{d}/uploads", files={"file": ("x.txt", b"first", "text/plain")})
    archive = _zip({"x.txt": b"different", "y.txt": b"new"})
    r = await alice.post(
        f"/domains/{d}/uploads", files={"file": ("p.zip", archive, "application/zip")}
    )
    b = (await alice.get(f"/domains/{d}/uploads/{r.json()['id']}")).json()
    assert b["status"] == "done"
...
async def test_archive_bomb_entry_count(alice, domain, monkeypatch):
    ...
    b = (await alice.get(f"/domains/{domain['id']}/uploads/{r.json()['id']}")).json()
...
async def test_index_and_fulltext_search(alice, domain):
    ...
    assert (await alice.get(f"/domains/{d}/documents?q=оборудования")).json()["total"] == 0

    idx = await alice.post(f"/documents/{doc['id']}/index")
    assert idx.json()["index_status"] == "done"
    assert idx.json()["text_source"] == "parsed"
    assert idx.json()["indexed_at"] is not None

    hit = await alice.get(f"/domains/{d}/documents?q=оборудования")
    assert [x["id"] for x in hit.json()["items"]] == [doc["id"]]

    assert (await alice.get(f"/domains/{d}/documents?has_index=true")).json()["total"] == 1
    assert (await alice.get(f"/domains/{d}/documents?has_index=false")).json()["total"] == 0
...
async def test_search_filters_and_facets(alice, domain):
    ...
    for name, ct in uploads:
        r = await alice.post(f"/domains/{d}/uploads", files={"file": (name, name.encode(), ct)})
        did = r.json()["document"]["id"]
        if name == "b.txt":
            await alice.patch(f"/documents/{did}/tags", json={"tag_names": ["invoice"]})

    txt = await alice.get(f"/domains/{d}/documents?ext=txt")
    assert txt.json()["total"] == 2

    tagged = await alice.get(f"/domains/{d}/documents?tags=invoice")
    assert tagged.json()["total"] == 1

    facets = (await alice.get(f"/domains/{d}/documents")).json()["facets"]
...
async def test_export_builds_zip_with_manifest(alice, domain):
    ...
    exp = await alice.post(f"/domains/{d}/exports", json={})
    assert exp.status_code == 202
    art_id = exp.json()["id"]

    art = (await alice.get(f"/artifacts/{art_id}")).json()
    assert art["status"] == "ready"
    assert art["item_count"] == 2

    dl = await alice.get(f"/artifacts/{art_id}/download")
...
async def test_export_by_id_list(alice, domain):
    ...
    exp = await alice.post(f"/domains/{d}/exports", json={"document_ids": ids[:2]})
    art = (await alice.get(f"/artifacts/{exp.json()['id']}")).json()
    assert art["item_count"] == 2


async def test_artifact_hidden_from_non_member(alice, bob, domain):
    d = domain["id"]
    await alice.post(f"/domains/{d}/uploads", files={"file": ("z.txt", b"z", "text/plain")})
    art_id = (await alice.post(f"/domains/{d}/exports", json={})).json()["id"]
    assert (await bob.get(f"/artifacts/{art_id}")).status_code == 404
